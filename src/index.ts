...
// REDUCER
const MainDispatcher = async (data: RootAction, wsInstance: ws, rootWs: Server): Promise<void> => {
  switch (data.type) {
    case getType(RootActions.CMD_RECIPES): {
      const recipes = await recipeMaker.getRecepies();
      WebSocketUtils.sendMessage(wsInstance, ServerActions.RECIPES, { recipes });
      break;
    }
    case getType(RootActions.CMD_EDIT): {
      ...
      WebSocketUtils.broadcastMessage(rootWs, ServerActions.RECIPES, { recipes: editRecepies });
      break;
    }
    case getType(RootActions.CMD_NEW): {
      const recipe = await recipeMaker.createRecipe();
      WebSocketUtils.sendMessage(wsInstance, ServerActions.NEW, { recipe });
      break;
    }
    case getType(RootActions.CMD_DELETE): {
      ...
      const recipes = await recipeMaker.getRecepies();
      WebSocketUtils.broadcastMessage(rootWs, ServerActions.RECIPES, { recipes });
      break;
    }
    case getType(RootActions.CMD_FAMILIES): {
      const families = await recipeMaker.getFamilies();
      WebSocketUtils.sendMessage(wsInstance, ServerActions.FAMILIES, { families });
      break;
    }
