...
export class RecipeService {
  ...
  public async setFamily(id: string): Promise<void> {
    const recipeFamily = await this.db
      .get(Collection.FAMILIES)
      .find({ id })
      .value();
    this.recipeFamily = recipeFamily;
  }

  public async upsertFamily(family: RecipeFamily): Promise<void> {
    const { id } = family;
  ...
  public async upsertRecipe(recipe: Recipe): Promise<void> {
    const { id } = recipe;
    const found = await this.db.get(Collection.recipes).find({ id });
    if (!found.value()) {
      await this.db
      ...
      await found.assign(recipe).write();
    }
  }

  public async delRecipe(id: string): Promise<void> {
    await this.db
      .get(Collection.recipes)
      .remove({ id })
      .write();
  ...
  public async setPumps(recipe: Recipe): Promise<void> {
    if (!this.executing && recipe) {
      this.executing = true;
      const { parts } = recipe;
      const promises: Promise<void>[] = parts.map((quantity: number, indx: number): Promise<void> => {
